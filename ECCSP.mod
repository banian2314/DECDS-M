 ...
 * OPL 12.9.0.0 Model
 * Author: banian
 * Creation Date: 2024年9月19日 at 下午4:29:48
 *********************************************/
int n = 6;//订单数
range jobs = 0..n;
int m = 2;//流水车间机器数
...
int Z1[1..2][1..2]=[[1,2],[5,6]];//配对订单集合
int Z2[1..2]=[4,3];//配对工序集合
dvar float T1;//辅助变量
dvar float T2;//辅助变量
dvar boolean u1;
dvar boolean u2;
...
subject to{
  ...
  //Constraint 14 虚拟工件分配在所有阶段(包括虚拟阶段)的所有机器上
    ...
    //Constraint 23
	forall(l in 1..L1) T1>=d[5][0]+P2[5][1]/v[1][l]+P2[6][1]/v[1][l]+(Y[5][l][1]-1)*h2;
	forall(l in 1..L2) T2>=d[1][1]+P2[1][2]/v[2][l]+P2[2][2]/v[2][l]+(Y[1][l][2]-1)*h2;
	//Constraint 23
	forall(l in 1..L1) T1>=d[6][0]+P2[6][1]/v[1][l]+P2[5][1]/v[1][l]+(Y[6][l][1]-1)*h2;
	forall(l in 1..L2) T2>=d[2][1]+P2[2][2]/v[2][l]+P2[1][2]/v[2][l]+(Y[2][l][2]-1)*h2;
	//Constraint 24
	forall(l in 1..L1) T1<=d[5][0]+P2[5][1]/v[1][l]+P2[6][1]/v[1][l]+(1-Y[5][l][1])*h2+M*(1-u5);
	forall(l in 1..L2) T2<=d[1][1]+P2[1][2]/v[2][l]+P2[2][2]/v[2][l]+(1-Y[1][l][2])*h2+M*(1-u7);
	//Constraint 24 
	forall(l in 1..L1) T1<=d[6][0]+P2[6][1]/v[1][l]+P2[5][1]/v[1][l]+(1-Y[6][l][1])*h2+M*(1-u6);
    forall(l in 1..L2) T1<=d[2][1]+P2[2][2]/v[2][l]+P2[1][2]/v[2][l]+(1-Y[2][l][2])*h2+M*(1-u8);
    //Constraint 25
    u5+u6>=1;
    u7+u8>=1;
    //Constraint 26
	...
	d[6][1]>=T1;
	d[2][2]>=T2;
	//Constraint 28 前提是在当前阶段中，共用同一台机器
